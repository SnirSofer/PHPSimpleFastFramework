<?php
	...
	function cutstring($string,$length = 17)
	{
		$string = (strlen($string) > $length ? substr($string,0,$length).".." : $string);
		return $string;
	}
	function text2safety($string) {
	...
	function timeAgo($a) {
		...
		$d = array( $lang -> get('year'), $lang -> get('month'), $lang -> get('week'), $lang -> get('day'), $lang -> get('hour'), $lang -> get('minute'));
		$e = array( $lang -> get('years'), $lang -> get('months'), $lang -> get('weeks'), $lang -> get('days'), $lang -> get('hours'), $lang -> get('minutes'));
		if( $b < 60 ) return $lang -> get('less-then-a-mintue');
		for( $f=0; ( $h = $b/$c[$f] ) <= 1 && $f < 5; $f++);
		return ( floor($h) > 1 ) ? sprintf( "%d %s", $h, $e[$f] ) : $d[$f];
	}
	
	function formatBytes($bytes, $precision = 2) { 
		$unit = ["B", "KB", "MB", "GB"]; $exp = floor(log($bytes, 1024)) | 0; 
